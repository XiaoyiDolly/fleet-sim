...
from .baseline_q_network import DeepQNetwork, FittingDeepQNetwork
from agent.dispatch_policy import DispatchPolicy
from . import settings
from common.time_utils import get_local_datetime
from common import vehicle_status_codes, mesh

class BaselineDispatchPolicy(DispatchPolicy):
...
class BaselineDispatchPolicyLearner(BaselineDispatchPolicy):
    ...
    def train_network(self, batch_size, n_iterations=1):
        ...
        for _ in range(n_iterations):
            ...
            for _ in range(batch_size):
                main_features, aux_features, y = self.replay_memory()
                main_feature_batch.append(main_features)
                aux_feature_batch.append(aux_features)
                y_batch.append(y)

            loss_sum += self.q_network.fit(main_feature_batch, aux_feature_batch, y_batch)
    ...
    def replay_memory(self, max_retry=100):
        for _ in range(max_retry):
            ...
            # return sa, y
            main_features = self.feature_constructor.construct_main_maps(l[0], l[1], sd)
            aux_features = self.feature_constructor.construct_aux_maps(l[0], l[1], t)
            target_features, _ = self.feature_constructor.construct_features(next_t, next_f, next_l, next_sd)
            target_value = self.q_network.compute_target_value(target_features)
            discount_factor = settings.GAMMA ** int((next_t - t) / 60)
            y = reward + discount_factor * target_value
            return main_features, aux_features, y
